...
public class AluguelRestControllerTest extends LocadoraApplicationTests {
    ...
    public void aluguelRetiradaTest() throws Exception{
        ...
        restMockMvc.perform(MockMvcRequestBuilders.post("/api/aluguel/retirada")
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$.retirada")
                        .value(dateService.dateTimeNow().format(ofPattern("dd/MM/yyyy HH:mm:ss"))))
                .andExpect(MockMvcResultMatchers.jsonPath("$.previsao")
                        .value(dateService.dateNow().plusDays(retirada.getMidias().size()).atStartOfDay().plusHours(16)
        ...
        Assert.assertEquals(dateService.dateTimeNow().format(ofPattern("dd/MM/yyyy  HH:mm"))
                                                ,alugueis.get(0).getRetirada().format(ofPattern("dd/MM/yyyy  HH:mm")));
        Assert.assertEquals(dateService.dateNow().plusDays(1).atStartOfDay().plusHours(16).format(ofPattern("dd/MM/yyyy HH:mm:ss"))
    ...
    public void aluguelDevolucaoComMultaTest() throws Exception{
        Cliente cliente = clienteRepository.save(Cliente.builder().nome("nome").rua("rua").bairro("bairro").cidade("cidade").estado("RS").telefone("9999").build());
    ...
    @WithMockUser(username="admin.admin", 
            password = "jwtpass", 
            authorities = {"ADMIN_USER"})
    public void aluguelDevolucaoHojeTest() throws Exception{
        Cliente cliente = clienteRepository.save(Cliente.builder().nome("nome").rua("rua").bairro("bairro").cidade("cidade").estado("RS").telefone("9999").build());
        ...
        Aluguel aluguel = aluguelRepository.save(Aluguel.builder()
                                                            .retirada(dateService.dateNow().minusDays(1).atTime(10, 0, 0))
                                                            .previsao(dateService.dateNow().atTime(16, 0, 0))
                                                            .idCliente(cliente)
                                                        .build());
        ...
        ValorMidia valor = valorMidiaRepository.save(ValorMidia.builder()
                                                                .valor(1.9d)
                                                                .inicioVigencia(dateService.dateNow().minusDays(2).atTime(6, 0))
                                                                .midia(midia)
                                                            .build());
        ...
        restMockMvc.perform(MockMvcRequestBuilders.get("/api/aluguel/devolucao?page=0&size=10&sort=id,desc")
                ...
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
                .andExpect(MockMvcResultMatchers.jsonPath("$.content.[0].id").value(alugueis.get(0).getId()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.content.[0].retirada").value(alugueis.get(0).getRetirada().format(ofPattern("dd/MM/yyyy HH:mm:ss"))))
                .andExpect(MockMvcResultMatchers.jsonPath("$.content.[0].previsao").value(alugueis.get(0).getPrevisao().format(ofPattern("dd/MM/yyyy HH:mm:ss"))))
                .andExpect(MockMvcResultMatchers.jsonPath("$.content.[0].devolucao").isEmpty())
                .andExpect(MockMvcResultMatchers.jsonPath("$.content.[0].valor").value(valor.getValor()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.content.[0].multa").value(0.0d));
