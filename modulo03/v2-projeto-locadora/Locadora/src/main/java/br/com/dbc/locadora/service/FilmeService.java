...
public class FilmeService extends AbstractCrudService<Filme> {
    ...
    public Long countByTipo (Long id, TipoMidia tipo){
        ...
        return midiaService.countByTipoAndIdFilme(tipo, filme.getId());
    }
    ...
    public Page<FilmeCatalogoDTO> searchCatalogo(Pageable pageable, FilmeCatalogoDTO dto){
        
        // Buscar os filmes
        List<Filme> listaDeFilmes = 
                filmeRepository.findByTituloOrCategoriaOrLancamentoBetweenIniAndFim(
                        pageable, dto.getTitulo(), dto.getCategoria(), dto.getLancamento(), dto.getLancamento()).getContent();
    
        List<FilmeCatalogoDTO> retorno = new ArrayList<>(); 
        
        for(Filme filme : listaDeFilmes){
            FilmeCatalogoDTO filmeCatatalogo = FilmeCatalogoDTO.builder().titulo(filme.getTitulo()).categoria(filme.getCategoria()).lancamento(filme.getLancamento()).build();

            filmeCatatalogo.setMidias( midiaService.midiaCatalogoListFromFilme(filme) );
            retorno.add(filmeCatatalogo);
        }
    
        return new PageImpl<FilmeCatalogoDTO>(
                        retorno,
                        pageable, 
                        retorno.size());
    }
